...
use App\Models\SoalUjianSaintek;
...
class SoalUjianSaintekController extends Controller
{
    public function index(Request $request)
    {
        $soalUjianSaintek = SoalUjianSaintek::get();

        if ($request->ajax()) {
            return DataTables::of($soalUjianSaintek)
                ->addIndexColumn()
                ...
                ->editColumn('soal_gambar', function ($item) {
                    if ($item->soal_gambar) {
                        $imagePath = "storage/soal-saintek/{$item->soal_gambar}";
                ...
                ->editColumn('soal_audio', function ($item) {
                    if ($item->soal_audio) {
                        $audioPath = "storage/soal-saintek/{$item->soal_audio}";
                ...
                ->addColumn('actions', function ($item) {
                    ...
                    <div class="dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-100px py-4" data-kt-menu="true">
                        <div class="menu-item px-3">
                            <a href="' . route('admin.soal-ujian-saintek.edit', $item->id) . '" class="menu-link px-3">
                                Edit Soal
                            </a>
                        </div>
                        <div class="menu-item px-3">
